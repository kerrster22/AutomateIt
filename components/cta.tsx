...
export function CtaPrimary({ href, children, className = "" }: CtaProps) {
  return (
    <Link
      href={href}
      className={`inline-flex items-center justify-center min-h-14 px-7 rounded-md bg-btn text-btnink text-[17px] font-semibold transition-opacity duration-150 hover:opacity-[0.86] ${className}`}
    >
      {children}
    </Link>
  );
